...
- 支持版本：1.20.1 Forge（本分支）/ 1.21.1 NeoForge（`1.21.1` 分支）
- 依赖：Create 6.0.x、Beyond Dimensions 0.7.x（硬依赖，缺失时阻止加载）
- 许可证：MIT

## 构建

需要 JDK 17（含 javac），依赖经 Modrinth Maven 自动下载：

```bash
./gradlew build
# 产物：build/libs/create-beyond-stock-fix-1.20.1-forge-1.0.0.jar
```

开发运行（自动带上 Create、BeyondDimensions等依赖）：
